...
E[2]="The script must be run as root, you can enter sudo -i and then download and run again. Feedback: [https://t.me/LumaPixie]"
C[2]="必须以 root 方式运行脚本，可以输入 sudo -i 后重新下载运行，问题反馈:[https://t.me/LumaPixie]"
E[3]="Unsupported architecture: \$(uname -m)"
C[3]="不支持的架构: \$(uname -m)"
E[4]="Please choose: "
C[4]="请选择: "
E[5]="The script supports Linux systems only. Feedback: [https://t.me/LumaPixie]"
C[5]="本脚本只支持 Linux 系统，问题反馈:[https://t.me/LumaPixie]"
...
get_local_version() {
  ...
  if [ -L "$WORK_DIR/nodepass" ]; then
    ...
    if [[ "$SYMLINK_TARGET" == *"np-stb"* ]]; then
      ...
      VERSION_TYPE_TEXT=$(text 66)
    elif [[ "$SYMLINK_TARGET" == *"np-lts"* ]]; then
      VERSION_TYPE_TEXT=$(text 98)
    fi
  fi
...
switch_version() {
  ...
  local current_info=$(get_local_version)
  local current_type=$(echo "$current_info" | cut -d'|' -f1)
  local current_version=$(echo "$current_info" | cut -d'|' -f2)
 
  echo ""
  info " $(text 97) $current_type $current_version"
  echo ""
  ...
  if start_service; then
    local new_info=$(get_local_version)
    local new_type=$(echo "$new_info" | cut -d'|' -f1)
    local new_version=$(echo "$new_info" | cut -d'|' -f2)
    info " $(text 87)"
    info "$new_type $new_version"
  else
    warning " $(text 89) "
    return 1
  fi
...
upgrade_core() {
  ...
  echo ""
  echo " $(text 110) $upgrade_info"
  echo ""
 
  read -p " $(text 48) " UPGRADE_CHOICE
  if [ "${UPGRADE_CHOICE,,}" != "y" ]; then
    info " $(text 49) "
    exit 0
  fi
  ...
  for file in "${upgrade_files[@]}"; do
    # 备份
    [ -f "$WORK_DIR/$file" ] && cp "$WORK_DIR/$file" "$WORK_DIR/$file.bak"
    # 升级
    cp "$OFFLINE_DIR/$file" "$WORK_DIR/$file"
    chmod +x "$WORK_DIR/$file"
    info " $(text 112) $file"
  ...
  if [ $NEED_RESTART -eq 1 ]; then
    ...
    if start_service; then
      ...
      # 清理备份
      for file in "${upgrade_files[@]}"; do
        rm -f "$WORK_DIR/$file.bak" 2>/dev/null
      done
    else
      warning " $(text 53) "
      # 回滚
      for file in "${upgrade_files[@]}"; do
        [ -f "$WORK_DIR/$file.bak" ] && mv "$WORK_DIR/$file.bak" "$WORK_DIR/$file"
      done
  ...
  if [ $status -ne 2 ]; then
    ...
    if [ -f "$WORK_DIR/data" ]; then
      ...
      if [ -n "$PROCESS_INFO" ]; then
        # 使用更精确的正则表达式匹配master://:PORT
        if echo "$PROCESS_INFO" | grep -q 'master://'; then
          # 提取端口 - 匹配 master://:PORT/ 格式
          PORT=$(echo "$PROCESS_INFO" | sed -n 's/.*master:\/\/:\([0-9]\+\).*/\1/p')
         
          # 如果没匹配到，尝试其他格式
          if [ -z "$PORT" ]; then
            # 尝试匹配 master://IP:PORT 格式
            PORT=$(echo "$PROCESS_INFO" | sed -n 's/.*master:\/\/[^:]*:\([0-9]\+\).*/\1/p')
          fi
        fi
       
        # 提取前缀 - 匹配 master://:PORT/PREFIX 格式
        PREFIX=$(echo "$PROCESS_INFO" | sed -n 's/.*master:\/\/:[0-9]\+\(\/[^?]\+\).*/\1/p')
        if [ -n "$PREFIX" ]; then
          PREFIX=$(echo "$PREFIX" | sed 's/^\///')  # 去掉开头的斜杠
        else
          # 尝试其他格式
          PREFIX=$(echo "$PROCESS_INFO" | sed -n 's/.*master:\/\/[^:]*:[0-9]\+\(\/[^?]\+\).*/\1/p')
          [ -n "$PREFIX" ] && PREFIX=$(echo "$PREFIX" | sed 's/^\///')
        fi
       
        # 提取TLS模式
        if echo "$PROCESS_INFO" | grep -q "tls=1\|tls=2"; then
          TLS_MODE=$(echo "$PROCESS_INFO" | grep -oE "tls=[0-2]" | cut -d= -f2)
        else
          TLS_MODE="0"
        fi
      fi
      ...
      if [ -z "$PORT" ] || [ -z "$PREFIX" ]; then
        if [ -f "/etc/systemd/system/nodepass.service" ]; then
          ...
          if [ -n "$SERVICE_CMD" ]; then
            ...
            [ -z "$PORT" ] && PORT=$(echo "$SERVICE_CMD" | sed -n 's/.*master:\/\/[^:]*:\([0-9]\+\).*/\1/p')
           
            # 提取前缀
            PREFIX=$(echo "$SERVICE_CMD" | sed -n 's/.*master:\/\/:[0-9]\+\(\/[^?]\+\).*/\1/p')
            if [ -n "$PREFIX" ]; then
            ...
            TLS_MODE=$(echo "$SERVICE_CMD" | grep -oE "tls=[0-2]" | cut -d= -f2 || echo "0")
          fi
        fi
      fi
     
      # 方法3：从data文件中的CMD获取
      if [ -z "$PORT" ] && [ -n "$CMD" ]; then
        PORT=$(echo "$CMD" | sed -n 's/.*master:\/\/:\([0-9]\+\).*/\1/p')
        [ -z "$PORT" ] && PORT=$(echo "$CMD" | sed -n 's/.*master:\/\/[^:]*:\([0-9]\+\).*/\1/p')
       
        PREFIX=$(echo "$CMD" | sed -n 's/.*master:\/\/:[0-9]\+\(\/[^?]\+\).*/\1/p')
        if [ -n "$PREFIX" ]; then
          PREFIX=$(echo "$PREFIX" | sed 's/^\///')
        else
          PREFIX=$(echo "$CMD" | sed -n 's/.*master:\/\/[^:]*:[0-9]\+\(\/[^?]\+\).*/\1/p')
        ...
        TLS_MODE=$(echo "$CMD" | grep -oE "tls=[0-2]" | cut -d= -f2 || echo "0")
      fi
      ...
      if [ -n "$REMOTE" ]; then
        ...
        if [[ "$REMOTE" =~ @?(.*):([0-9]+) ]]; then
          ...
          # 对于IPv6地址，添加方括号
          if [[ "$REMOTE_IP" =~ ^[0-9a-fA-F:]+$ ]] && [[ "$REMOTE_IP" =~ : ]]; then
      ...
      # 显示API信息
      if [ -n "$DISPLAY_IP" ] && [ -n "$PORT" ] && [ -n "$PREFIX" ]; then
        info " $(text 39) ${HTTP_S}://${DISPLAY_IP}:${PORT}/${PREFIX}/v1"
      else
      ...
      if [ -n "$KEY" ]; then
        ...
        warning " $(text 40) 未找到 API KEY"
      fi
     
      # 生成URI
      if [ -n "$DISPLAY_IP" ] && [ -n "$PORT" ] && [ -n "$KEY" ] && [ -n "$PREFIX" ]; then
        local API_URL="${HTTP_S}://${DISPLAY_IP}:${PORT}/${PREFIX}/v1"
        local URI="np://master?url=$(echo -n "$API_URL" | base64 -w0 2>/dev/null || echo "$API_URL" | base64)&key=$(echo -n "$KEY" | base64 -w0 2>/dev/null || echo "$KEY" | base64)"
        info " $(text 90) $URI"
      ...
      # 显示内网穿透信息
      if [ -n "$INSTANCE_ID" ] && [ -n "$KEY" ] && [ -n "$PORT" ] && [ -n "$PREFIX" ]; then
        # 尝试获取内网穿透实例信息
        if command -v curl >/dev/null 2>&1; then
          local INSTANCE_INFO=$(curl -ks "${HTTP_S}://127.0.0.1:${PORT}/${PREFIX}/v1/instances/${INSTANCE_ID}" -H "X-API-Key: ${KEY}" 2>/dev/null)
          if echo "$INSTANCE_INFO" | grep -q '"url"'; then
            echo ""
            info " $(text 82) "
            echo "$INSTANCE_INFO" | grep -o '"url":"[^"]*"' | cut -d'"' -f4
          fi
        fi
      fi
    else
      warning " $(text 39) 配置文件不存在"
      warning " $(text 40) 未找到配置文件"
    fi
  fi
...
parse_command() {
  ...
  case "$1" in
    -i|--install)
      warning " $(text 8) "
      warning "请运行: bash <(wget -qO1 https://raw.githubusercontent.com/lima-droid/npsh/main/np.sh) -i"
      ;;
    -u|--uninstall)
      uninstall_node
      ;;
    ...
    "")
      ...
      if [ $status -eq 2 ]; then
        ...
        case $choice in
          1)
            warning " $(text 8) "
            warning "运行: bash <(wget -qO1 https://raw.githubusercontent.com/lima-droid/npsh/main/np.sh) -i"
            ;;
          0) exit 0 ;;
          *) warning " $(text 17) " ;;
        esac
      else
        local version_info=$(get_local_version)
        local version_type=$(echo "$version_info" | cut -d'|' -f1)
        local version_num=$(echo "$version_info" | cut -d'|' -f2)
       
        if [ -n "$version_type" ] && [ -n "$version_num" ]; then
          info " $(text 97) $version_type $version_num"
          echo "--------------------------------------------"
        fi
...
╰───────────────────────────────────────────╯ "
  ...
  grep -qE '0|1' <<< "$INSTALL_STATUS" && info " $(text 60) $NODEPASS_STATUS "
  ...
  case "$1" in
    ...
    -s)
      ...
      if [ "$INSTALL_STATUS" = 2 ]; then
        ...
        get_api_key output
        if [ -n "$REMOTE" ] && [ -n "$INSTANCE_ID" ]; then
          get_intranet_penetration_server_cmd output
        fi
        get_uri output
